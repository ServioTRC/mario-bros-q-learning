...
class DQNAgent:
    ...
    def build_model(self):
        ...
        self.input = tf.compat.v1.placeholder(dtype=tf.float32, shape=() + self.states, name='input')
        ...
        with tf.compat.v1.variable_scope('online'):
            ...
            self.flatten = tf.reshape(self.conv_3, [tf.shape(input=self.conv_3)[0], -1])
            self.flatten.set_shape([1, 46592])
        ...
        with tf.compat.v1.variable_scope('target'):
            ...
            self.flatten = tf.reshape(self.conv_3_target, [tf.shape(input=self.conv_3_target)[0], -1])
            self.flatten.set_shape([1, 46592])
            self.dense_target = tf.compat.v1.layers.dense(inputs=self.flatten, units=512, activation=tf.nn.relu)
